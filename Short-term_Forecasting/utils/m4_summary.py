...
class M4Summary:
    ...
    def evaluate_special(self, group_name):
        """
        Evaluate forecasts using M4 test dataset.
        ...
        grouped_smapes = self.summarize_groups_special(grouped_smapes, group_name)
        grouped_mapes = self.summarize_groups_special(grouped_mapes, group_name)
        grouped_model_mases = self.summarize_groups_special(model_mases, group_name)
        grouped_naive2_smapes = self.summarize_groups_special(naive2_smapes, group_name)
        grouped_naive2_mases = self.summarize_groups_special(naive2_mases, group_name)
        for k in grouped_model_mases.keys():
            grouped_owa[k] = (grouped_model_mases[k] / grouped_naive2_mases[k] +
                              grouped_smapes[k] / grouped_naive2_smapes[k]) / 2
        ...
        return round_all(grouped_smapes), round_all(grouped_owa), round_all(grouped_mapes), round_all(
            grouped_model_mases), round_all(grouped_naive2_smapes), round_all(grouped_naive2_mases)
    ...
    def summarize_groups(self, scores):
        ...
        # average2 = (scores_summary['Yearly']*group_count('Yearly') + scores_summary['Quarterly']*group_count('Quarterly') + scores_summary['Monthly']*group_count('Monthly') + scores_summary['Others']*others_count) / (group_count('Yearly') + group_count('Quarterly') + group_count('Monthly') + others_count)

        return scores_summary
    
    def summarize_groups_special(self, scores, group_name):
        """
        Re-group scores respecting M4 rules.
        :param scores: Scores per group.
        :return: Grouped scores.
