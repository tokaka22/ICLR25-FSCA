...
parser.add_argument('--version_num', default='main02_01', type=str)
...
parser.add_argument('--data_flow', type=str, default='ETTh1_ETTh2')
...
for ii in range(args.itr):
    group_name = '{}2{}_{}_ps{}_{}_sl{}_df{}_{}*{}_s{}b{}l{}em{}rf{}mul{}ts{}wf{}wv{}id{}od{}dl{}s{}'.format(
        args.data,
        args.test_data,
        args.model, 
        args.patch_size,
    ...
    setting = 'e{}_v{}_r{}_nh{}_pl{}'.format(
        args.train_epochs,
        args.version_num, 
        args.run_name,
        args.n_heads,
    ...
    if args.lradj == 'COS':
        ...
        # scheduler_edge = torch.optim.lr_scheduler.CosineAnnealingLR(model_optim_edge_weights, T_max=args.tmax, eta_min=args.edge_eta_min)
    elif args.lradj == 'TST':
    ...
    for epoch in range(args.train_epochs):
        ...
        log = "Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Vali Loss: {3:.7f} Test Loss: {4:.7f}".format(epoch + 1, train_steps, train_loss, vali_loss, test_loss)
        print(log)
        with open(os.path.join(args.log_path, 'log.txt'), 'a') as f:
            f.write(log)
            f.write('\n')

        wandb.log({'Train Loss': train_loss, "Vali Loss": vali_loss, "Test Loss": test_loss, "w_l2s": model.w_l2s.item(), "epoch": epoch})
    ...
    if args.test_random:
        if args.test_random_type == 'un':
            randomize_weights_uniform(model.gpt2)
        elif args.test_random_type == 'ori':
            model.gpt2 = GPT2Model(GPT2Config())
            model.gpt2.h = model.gpt2.h[:args.gpt_layers]
            model.to(device=device)
        elif args.test_random_type == 'minmax':
            randomize_weights_uniform_minmax(model.gpt2)
