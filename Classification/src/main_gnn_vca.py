...
from running_vca import setup, pipeline_factory, validate, check_progress, NEG_METRICS
from utils import utils
from datasets.data_vca import data_factory, Normalizer
from datasets.datasplit import split_dataset
from models.ts_transformer import model_factory
from models.gpt4ts import gpt4ts
from models.GNNLLM_vca import GNNLLM_vca
...
def main(config):
    ...
    config['train_data_class'] = config['data_class'] + '_train'
    data_class = data_factory[config['train_data_class']]
    ...
    ###### example
    examples_df = my_data.examples_df
    examples_labels_df = my_data.examples_labels
    examples_indices = my_data.examples_indices

    examples = []
    examples_labels = []
    for idx in examples_indices:
        example = examples_df.loc[idx].values
        examples_label = examples_labels_df.loc[idx].values

        example = torch.from_numpy(example)
        examples_label = torch.from_numpy(examples_label)

        examples.append(example)
        examples_labels.append(examples_label)
    ...
    ### label unique index
    config['label_values'] = list(my_data.labels_df[0].unique())
    ...
    model = GNNLLM_vca(config, my_data)
    ...
    trainer = runner_class(model, train_loader, device, loss_module, optimizer, examples = examples, examples_labels = examples_labels, l2_reg=output_reg,
                                 print_interval=config['print_interval'], console=config['console'])
    val_evaluator = runner_class(model, val_loader, device, loss_module, examples = examples, examples_labels = examples_labels,
                                       print_interval=config['print_interval'], console=config['console'])
