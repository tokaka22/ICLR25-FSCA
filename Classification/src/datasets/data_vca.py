...
class TSRegressionArchive_train(BaseData):
    """
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        ### Identify unique classes and randomly select an example from each
        unique_classes = self.labels_df[0].unique()
        examples_indices = []

        for cls in unique_classes:
            class_indices = self.labels_df[self.labels_df[0] == cls].index
            example_index = np.random.choice(class_indices, 1, replace=False)
            examples_indices.append(example_index[0])
        
        # Storing examples separately
        self.examples_df = self.all_df.loc[examples_indices]
        self.examples_labels = self.labels_df.loc[examples_indices]
        self.examples_indices = examples_indices

        self.all_df['original_index'] = self.all_df.index

        self.labels_df['original_index'] = self.labels_df.index
        
        # Removing examples from the original dataframes
        self.all_df = self.all_df.drop(examples_indices)
        self.labels_df = self.labels_df.drop(examples_indices)

        index_map = {idx: i for i, idx in enumerate(sorted(set(self.all_df['original_index'])))}
        self.all_df['continuous_index'] = self.all_df['original_index'].map(index_map)

        index_map = {idx: i for i, idx in enumerate(sorted(set(self.labels_df['original_index'])))}
        self.labels_df['continuous_index'] = self.labels_df['original_index'].map(index_map)

        self.all_df = self.all_df.set_index('continuous_index')
        self.all_df.index.name = None
        self.all_df = self.all_df.drop(columns=['original_index'])

        self.labels_df = self.labels_df.set_index('continuous_index')
        self.labels_df.index.name = None
        self.labels_df = self.labels_df.drop(columns=['original_index'])
...
data_factory = {'weld': WeldData,
                ...
                'tsra_train': TSRegressionArchive_train,
                'pmu': PMUData}
