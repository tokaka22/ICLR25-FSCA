...
def collate_superv01(data, max_len=None):
    ...
    batch_size = len(data)
    features_b, labels_b, features, labels, IDs = zip(*data)

    ###### features_b, labels_b
    # Stack and pad features and masks (convert 2D to 3D tensors, i.e. add batch dimension)
    lengths = [X.shape[0] for X in features_b]  # original sequence length for each time series
    if max_len is None:
        max_len = max(lengths)
    X_b = torch.zeros(batch_size, max_len, features_b[0].shape[-1])  # (batch_size, padded_length, feat_dim)
    for i in range(batch_size):
        end = min(lengths[i], max_len)
        X_b[i, :end, :] = features_b[i][:end, :]

    targets_b = torch.stack(labels_b, dim=0)  # (batch_size, num_labels)
    ...
    return X_b, targets_b, X, targets, None, IDs
...
class ClassiregressionDataset01(Dataset):

    def __init__(self, data, indices):
        super(ClassiregressionDataset01, self).__init__()
    ...
    def __getitem__(self, ind):
        ...
        X_b = self.feature_df.loc[self.IDs[ind]].values  # (seq_length, feat_dim) array
        y_b = self.labels_df.loc[self.IDs[ind]].values  # (num_labels,) array

        X = self.feature_df.loc[self.IDs[ind+1]].values  # (seq_length, feat_dim) array
        y = self.labels_df.loc[self.IDs[ind+1]].values  # (num_labels,) array


        return torch.from_numpy(X_b), torch.from_numpy(y_b), torch.from_numpy(X), torch.from_numpy(y), self.IDs[ind]

    def __len__(self):
        return len(self.IDs) - 1


class ClassiregressionDataset_example(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        X1 = self.feature_df.loc[self.IDs[ind]].values  # (seq_length, feat_dim) array
        y2 = self.labels_df.loc[self.IDs[ind]].values  # (num_labels,) array
...
class ClassiregressionDataset_GNN(Dataset):
