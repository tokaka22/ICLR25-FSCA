...
# from embed import DataEmbedding, DataEmbedding_wo_time, DataEmbedding_wo_time_wp
...
def build_edges(sequence):
    ...
    e0 p l0 e1 p l1 x p
    '''
    # Indices where each value starts
    indices_1 = np.where(sequence == 1)[0]
    indices_2 = np.where(sequence == 2)[0]
    indices_3 = np.where(sequence == 3)[0]

    indices_4 = np.where(sequence == 4)[0]
    indices_5 = np.where(sequence == 5)[0]
    indices_6 = np.where(sequence == 6)[0]

    indices_7 = np.where(sequence == 7)[0]
    indices_8 = np.where(sequence == 8)[0]

    assert len(indices_3) == 1
    assert len(indices_6) == 1

    # example 0
    edges_1_to_2 = np.array([[i, indices_2[0]] for i in indices_1])  # All 1s to the first 2
    edges_2_to_3 = np.array([[indices_2[-1], i] for i in indices_3]) # Last 2 to 3 

    # example 1
    edges_4_to_5 = np.array([[i, indices_5[0]] for i in indices_4])  
    edges_5_to_6 = np.array([[indices_5[-1], i] for i in indices_6])      
    
    # x
    edges_7_to_8 = np.array([[i, indices_8[0]] for i in indices_7])  

    # Concatenate all edges
    edges = np.concatenate([edges_1_to_2, edges_2_to_3, edges_4_to_5, edges_5_to_6, edges_7_to_8])

    index_list = np.concatenate([
        np.zeros(len(edges_1_to_2), dtype=int),  # Mark positions for edges_1_to_2 as 0
        np.ones(len(edges_2_to_3), dtype=int) * 1,
        np.ones(len(edges_4_to_5), dtype=int) * 2,
        np.ones(len(edges_5_to_6), dtype=int) * 3,
        np.ones(len(edges_7_to_8), dtype=int) * 4,
    ])

    return edges, index_list


def construct_sequence(L_p, L_fix_prompt, s):
    '''
    e0 p l0 e1 p l1 x p
    '''
    ...
    array = np.concatenate([
        np.full(L_p, 1),           # e0
        np.full(L_fix_prompt, 2),  # p
        np.array([3]),  
        np.full(L_p, 4),           # e1
        np.full(L_fix_prompt, 5),  # p
        np.array([6]),  
        np.full(L_p, 7),           # x
        np.full(L_fix_prompt, 8),  # p
    ])
