...
from datasets.dataset import ClassiregressionDataset_GNN
import wandb
...
# def pipeline_factory_gnn(config):
#     """For the task specified in the configuration returns the corresponding combination of
#     Dataset class, collate function and Runner class."""
...
#                        start_hint=config['start_hint'], end_hint=config['end_hint']), collate_unsuperv, UnsupervisedRunner
#     if (task == "classification") or (task == "regression"):
#         return ClassiregressionDataset_GNN, collate_superv, SupervisedRunner
#     else:
#         raise NotImplementedError("Task '{}' not implemented".format(task))
...
class SupervisedRunner(BaseRunner):
    ...
    def __init__(self, *args, examples=None, examples_labels=None, **kwargs):
        ...
        self.examples = [example.to(self.device, dtype=torch.float32) for example in examples]
        self.examples_labels = [examples_label.to(self.device, dtype=torch.float32) for examples_label in examples_labels]
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            predictions = self.model(self.examples, self.examples_labels, X.to(self.device), padding_masks)
